...
export function TableOfContents({ content }: { content: string }) {
    ...
    useEffect(() => {
        // Find H2 and H3 tags in markdown
        // Regex for H2: /^##\s+(.+)$/m
        // Regex for H3: /^###\s+(.+)$/m
    ...
    return (
        <nav className="space-y-4 p-5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 not-prose mb-8">
            ...
            <ul className="space-y-2 border-l border-slate-200 dark:border-slate-700 ml-2">
                {toc.map((item, i) => (
                    <li key={i} className={`${item.level === 3 ? 'ml-4' : '-ml-px'} border-l border-transparent hover:border-blue-500`}>
                        <a 
                            href={`#${item.id}`} 
                            className={`block py-1 px-3 text-sm transition-colors ${item.level === 2 ? 'font-bold text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-400 hover:text-blue-500'}`}
                            onClick={(e) => {
                                e.preventDefault()
