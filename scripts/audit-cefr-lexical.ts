...
async function auditCefrLexical() {
    ...
    // --- 3.2 LEXICAL BLOCKER PERSISTENCE ---
    console.log("\n[3.2] Lexical Blocker Persistence Test");
    const blockerObj = {
        level_capped: true,
        category: "Vocabulary",
    ...
    await updateUserFluencyProfile(TEST_USER_ID, {
        ...
        session_type: "AI_TUTOR",
        source_session_id: "audit-lexical-1",
        speaking_time_seconds: 120
    });

    // Verify blocker persisted in profile
    const updatedProfile = await prisma.user_fluency_profile.findUnique({ where: { user_id: TEST_USER_ID } });
    const blockers = updatedProfile?.lexical_blockers as any;

    if (blockers && blockers.level_capped === true && blockers.category === "Vocabulary") {
        console.log("✅ PASS: Lexical blocker persisted in user_fluency_profile.");
    } else {
        console.error("❌ FAIL: Lexical blocker not found in profile.");
    }
