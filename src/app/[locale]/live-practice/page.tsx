...
export default function LivePracticePage() {
  ...
  const [lastReportSessionId, setLastReportSessionId] = useState<string | null>(
    null,
  );
  ...
  const fetchReportWithRetry = useCallback(async (sessionId: string) => {
    ...
    setIsLoadingReport(false);
    setError(
      "Your feedback is still being prepared. Please check again in a moment.",
    );
  }, []);
  ...
  const pollForMatch = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to join queue (${response.status})`,
        );
      }
                ...
                {report && (
                  <div className="space-y-3">
                      ...
                      <div className="rounded-lg bg-white dark:bg-slate-800 p-3 border border-slate-200 dark:border-slate-700">
                        ...
                        <div className="space-y-1 text-sm text-slate-600 dark:text-slate-300">
                          <p>
                            ...
                            {report.aiReport.insights.fluency}
                          </p>
                          <p>
                            <span className="font-medium">Grammar:</span>{" "}
                            {report.aiReport.insights.grammar}
                          </p>
                          <p>
                            <span className="font-medium">Vocabulary:</span>{" "}
                            {report.aiReport.insights.vocabulary}
                          </p>
                        </div>
                      </div>
                    )}

                    {Array.isArray(report.aiReport?.refinements) &&
                      report.aiReport.refinements.length > 0 && (
                        <div className="rounded-lg bg-white dark:bg-slate-800 p-3 border border-slate-200 dark:border-slate-700">
                          <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 mb-2">
                            A few helpful refinements
                          </p>
                          <div className="space-y-2">
                            {report.aiReport.refinements
                              .slice(0, 3)
                    ...
                    {lastReportSessionId && (
                      <button
                        onClick={() =>
                          fetchReportWithRetry(lastReportSessionId)
                        }
          ...
          <div className="fixed inset-0 bg-white dark:bg-slate-900 z-50 flex flex-col items-center justify-between p-6 animate-in fade-in duration-300">
            ...
            <div className="w-full flex justify-between items-center max-w-lg mt-8">
              <div className="flex items-center gap-2 bg-green-100 dark:bg-green-900/30 px-3 py-1.5 rounded-full">
                ...
                <span className="text-green-700 dark:text-green-300 text-xs font-bold tracking-wide uppercase">
                  Live
                </span>
              </div>
